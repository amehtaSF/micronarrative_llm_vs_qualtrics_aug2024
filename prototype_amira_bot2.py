...
part1_table = dynamodb.Table('petr_micronarrative_nov2024')
part2_table = dynamodb.Table('petr_micronarrative_nov2024_2')
...
def getData (testing = False ): 
    ...
    if len(msgs.messages) == 0:
        ...
        ## original line here  -- if we didn't need to pipe in the previous story
                # msgs.add_ai_message(llm_prompts.questions_intro)

        # ADDED CODE -- ASHISH
        # Query dynamo to get the last story from the user
        db_response = part1_table.query(KeyConditionExpression=boto3.dynamodb.conditions.Key('chat_id').eq(prolific_id))
        items = db_response.get('Items', [])
        story_from_db = items[-1].get('scenario', None) if items else "No story found -- please contact researcher on Prolific"
